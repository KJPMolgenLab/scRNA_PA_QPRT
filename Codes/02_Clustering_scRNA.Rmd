...
dir.create("./Results/02_Clustering/", showWarnings = F)
...
ggsave("./Results/02_Clustering/Clustering_Resopt.svg", p1)
```
...
p1|p2
ggsave("./Results/02_Clustering/Clustering_integrated_clustree.svg", p1|p2)
...
p1
ggsave("./Results/02_Clustering/Clustering_UmapParamOpt.svg", p1)
...
p2 = DimPlot(seurat_integrated, 
             ...
             label = T)
p2
ggsave("./Results/02_Clustering/Clustering_final_optimized_all.svg",p2)
```
...
p2 = DimPlot(seurat_integrated, 
             ...
             label = T)
p2
ggsave("./Results/02_Clustering/Clustering_UmapByType.svg")
```
...
ggsave("./Results/02_Clustering/Clustering_CellcycleScoring.svg", p1|p2)
```
...
cdist
ggsave("./Results/02_Clustering/Clustering_Distribution.svg")
```
...
qcplot<- FeaturePlot(seurat_integrated, 
            ...
            min.cutoff = 'q10')
qcplot
ggsave("./Results/02_Clustering/Clustering_FeatPlot_QCMeasures.svg", qcplot)
```
...
ggsave("./Results/02_Clustering/Clustering_PCAvsCluster.svg", p1)
```
...
openxlsx2::write_xlsx(markers,"./Results/02_Clustering/Cluster_markers.xls")
...
p1
ggsave("./Results/02_Clustering/Clustering_Heatmap_Markers.svg")
```
...
p
ggsave("./Results/02_Clustering/Clustering_NeuronalMarkers.svg", p)
```
...
openxlsx2::write_xlsx(tablegostres, "./Results/02_Clustering/Clusters_GO_termsCluster.xlsx")
```
...
p
ggsave("./Results/02_Clustering/Clustering_KnoblichPredTypes.svg",p)
```
...
p|p2
ggsave("./Results/02_Clustering/Clustering_KnoblMapped.svg",p)
...
p
ggsave("./Results/02_Clustering/Clustering_KnoblScores.svg")
```
...
p
ggsave("./Results/02_Clustering/Clustering_KantonT4PredTypes.svg")

p<-DimPlot(data[,data$KantonT4PredType=="choroid plexus"], group.by = "KantonT4PredType", split.by = "type")
p
ggsave("./Results/02_Clustering/Clustering_KantonT4PredTypesChoroidOnly.svg")
```
...
p<- DimPlot(seurat_integrated, group.by = "Hum_dev.labels", label = T)

p2<- DimPlot(seurat_integrated, group.by = "seurat_clusters", label = T)
p|p2
ggsave("./Results/02_Clustering/Clustering_Hum_devMapped.svg",p)

table(seurat_integrated$seurat_clusters, seurat_integrated$Hum_dev.labels)
...
p<-FeaturePlot(seurat_integrated, features = feats)+NoLegend()
p
ggsave("./Results/02_Clustering/Clustering_Hum_devScores.svg")
```
