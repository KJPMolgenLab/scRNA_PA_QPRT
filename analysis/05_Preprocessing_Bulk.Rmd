...
Countdata <- Countdata[,SampleInfo$PAmM==0]
SampleInfo <- SampleInfo[SampleInfo$PAmM==0,]
...
cutN=5 #number of different conditions (2xPA, 5xDay)
...
par(mar=c(15,3,5,3))
plot(as.dendrogram(hc), main=paste("Similarity by gene expression, guessed",cutN,"clusters"), cex=0.7)
...
```{r PCA_MDS, fig.width=8, fig.height=4}
...
ppca <- ggplot(gpca.dat, aes(x = dim1, y = dim2, color = as.factor(Day), pch=Batch)) +
            geom_point(size = 2)  + ggtitle("PCA with log2 counts")+theme_classic()

pmds <- ggplot(mds, aes(x = `1`, y = `2`, color = as.factor(Day), pch=Batch)) +
            geom_point(size = 2)  + ggtitle("MDS with log2 counts")+theme_classic()


pcmb <- ggpubr::ggarrange(ppca,pmds,common.legend = T, legend = "bottom")

pcmb

ggsave("/files/output/05_Bulk_Preprocessing/05_Bulk_Preprocessing_PCA.svg", pcmb)
