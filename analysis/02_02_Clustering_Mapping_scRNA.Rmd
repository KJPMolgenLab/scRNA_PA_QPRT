...
geneuniverses <- rownames(seurat_integrated)
...
```{r}
summ_lab<- openxlsx2::read_xlsx("./data/SummarizedCellTypes.xlsx")
summ_lab$Abbrev <- gsub("\\(", "", summ_lab$Abbrev)
summ_lab$Abbrev <- gsub("\\)", "_", summ_lab$Abbrev)
rownames(summ_lab) <- summ_lab$Orig_cluster
seuratmeta <- seurat_integrated@meta.data
seuratmeta$merges_clusters <- summ_lab[seuratmeta$seurat_clusters, "Abbrev"]
seuratmeta -> seurat_integrated@meta.data

p<- DimPlot(seurat_integrated, group.by = "merges_clusters")
ggsave(file="output/Clusterain_final_optimized_all_merged_lables.svg", p)
p
```


```{r, fig.height=6}
Idents(seurat_integrated) <- "merges_clusters"
DefaultAssay(seurat_integrated) = "RNA"
markers <- FindAllMarkers(object = seurat_integrated, 
                          only.pos = TRUE,
                          logfc.threshold = 0.25)  

openxlsx2::write_xlsx(markers,"./output/02_Clustering/ClustersMerged_markers.xls")
...
p1
ggsave("./output/02_Clustering/ClustersMerged_Heatmap_Markers.svg")
...
restab <- gostres$result


openxlsx2::write_xlsx(restab %>% dplyr::select(-parents), "/files/output/02_Clustering/ClustersMerged_GO_termsCluster.xlsx")
...
p
ggsave(file="./output/02_Clustering/ClustersMerged_Markers_BP_GOplot.svg", plot = p)
...
p <- ggpubr::ggarrange(plotlist = plotlist, common.legend = T, nrow = 5, ncol =3)
p
ggsave(file="./output/02_Clustering/ClustersMerged_Markers_TF_GOplot.svg", plot = p)
...
p <- ggpubr::ggarrange(plotlist = plotlist, common.legend = T, nrow = 5, ncol =3)
p
ggsave(file="./output/02_Clustering/ClustersMerged_Markers_GOplot.svg", plot = p)

```


```{r, fig.width=6, fig.height=15}
Genelist_test <- test_Genelist(mygenelist = genelist, 
                                      geneunivers = geneuniverses)

p <- plot_Genelist_test(Genelist_test)
p

ggsave(file="./output/02_Clustering/ClustersMerged_Markers_Gene_enrichment.svg", p)
openxlsx2::write_xlsx(Genelist_test, file="./output/02_Clustering/ClustersMerged_Markers_Gene_enrichment.xlsx", row_names=T)
