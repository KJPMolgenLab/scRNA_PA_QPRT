...
if(force.recalc | (! file.exists(file="/files/output/net.rds"))){ # skips recalculation of net object as it takes long... 
  ...
  saveRDS(net, file="/files/output/net.rds")
  saveRDS(powerTable, file="/files/output/powertable.rds")
} else {
  net = readRDS(file="/files/output/net.rds")
  powerTable = readRDS(file="/files/output/powertable.rds")
  powers = c(seq(1,10,by=1), seq(12,20, by=2));
  colors = c("blue", "red","black")
  # Will plot these columns of the returned scale free analysis tables
  plotCols = c(2,5,6,7)
...
svg("/files/output/WGCNA_labeled_Heatmap.svg")
...
MOI = gsub("ME", "", names(pvallist)) %>% gsub(".PAmM5_Day", "", .)
...
p <- ggpubr::ggarrange(plotlist = plotlist, common.legend = T, nrow = 5, ncol =3)
p
ggsave(file=paste0(output, "/06_Bulk_DEX_Modules_BP_GOplot.svg"), plot = p)
...
p <- ggpubr::ggarrange(plotlist = plotlist, common.legend = T, nrow = 5, ncol =3)
p
ggsave(file=paste0(output, "/06_Bulk_DEX_Modules_TF_GOplot.svg"), plot = p)
...
p <- ggpubr::ggarrange(plotlist = plotlist, common.legend = T, nrow = 5, ncol =3)
p
ggsave(file=paste0(output, "/06_Bulk_DEX_Modules_GOplot.svg"), plot = p)
