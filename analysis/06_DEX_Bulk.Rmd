...
openxlsx2::write_xlsx(l, file=paste0(output, "05_DEX_Bulk_LFC_results.xlsx"), row_names=T )
```
...
openxlsx::write.xlsx(resGO$result, file = paste0(output, "/06_DEX_Bulk_GO_results.xlsx"))
```
...
p
ggsave(file=paste0(output, "/06_DEX_Bulk_top10all_GOplot.svg"), plot = p)
```
...
p <- ggpubr::ggarrange(plotlist = plotlist, common.legend = T, nrow = 5, ncol =3)
p
ggsave(file=paste0(output, "/06_DEX_Bulk_top10_BP_GOplot.svg"), plot = p)
```
...
p <- ggpubr::ggarrange(plotlist = plotlist, common.legend = T, nrow = 5, ncol =3)
p
ggsave(file=paste0(output, "/06_DEX_Bulk_top10_TF_GOplot.svg"), plot = p)
```
...
ggpubr::ggexport(filename = paste0(output, "/05_DEX_Bulk_Siggenes_boxplots.pdf"), plotlist = pcomb, width = 14, height = 21, dpi = 300)
```
