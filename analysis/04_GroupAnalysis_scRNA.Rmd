...
openxlsx2::write_xlsx(l, file=paste0(output, "Biostat_DEG_SFARI.xlsx"), row_names=T)
```
...
openxlsx::write.xlsx(resGO, "./output/04_Biostat/Biostat_DEG_GO_Cluster.xlsx")
...
l = list(Sfari_All_test, Sfari_Syndromic_test)
openxlsx2::write_xlsx(l, file=paste0(output, "Biostat_DEG_SFARI_Cluster.xlsx"), row_names=T)
...
p
ggsave(filename = "./output/04_Biostat/Biostat_DEG_GO_clusters.svg")
```
...
lineages=grep("Lineage", colnames(metadata),value = T) 
...
plotlist=list()
for(lin in c("shared.pseudo",lineages)){
